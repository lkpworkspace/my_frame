...
//鼠标点击事件
void MyPersonList::mousePressEvent(QMouseEvent *event)
{
...
void MyPersonList::contextMenuEvent(QContextMenuEvent *event)
{
...
//删除组
void MyPersonList::slotDelGroup()
{
...
void MyPersonList::slotAddBuddy()
{
    ...
    buddy->sign->setText("easy coding on linux");   //设置个性签名
#else
    MyAddrInfo info;
    ...
    buddy->SetAddr(info);
    buddy->SetName("kpli");
    buddy->SetMark("easy coding on linux");
#endif
    QList<QListWidgetItem*> tem = groupMap.keys(currentItem);//当前组对应的项（包括组本身和好友）复制给tem
    //关键代码
    QListWidgetItem *newItem = new QListWidgetItem();       //创建一个newItem
...
//删除好友
void MyPersonList::slotDelBuddy()
{
    groupMap.remove(currentItem);  //移除该好友
    delete currentItem;            //删除
}
//重命名完成
void MyPersonList::slotRenameEditFshed()
{
