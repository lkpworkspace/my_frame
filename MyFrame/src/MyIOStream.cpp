...
void MyOutputStream::WriteBits( const void* inData, uint32_t inBitCount )
{
...
void MyOutputStream::Write( const MyVec3& inVector )
{
    Write( inVector.mX );
    Write( inVector.mY );
    Write( inVector.mZ );
...
void MyInputStream::Read( MyVec3& outVector )
{
    Read( outVector.mX );
    Read( outVector.mY );
    Read( outVector.mZ );
...
void MyOutputStream::Write( const MyQuaternion& inQuat )
{
...
void MyOutputStream::ReallocBuffer( uint32_t inNewBitLength )
{
    if( mBuffer == nullptr )
    {
...
void MyInputStream::ReadBits( void* outData, uint32_t inBitCount )
{
...
void MyInputStream::Read( MyQuaternion& outQuat )
{
    float precision = ( 2.f / 65535.f );
